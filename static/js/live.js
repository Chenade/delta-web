var object = {
    'licensePlate': '',
    'device': [],
    'marker':{},
    thread: {}
}

var theMarker = {};

var map = new L.Map('map_index');
L.control.scale({metric: true, imperial: false}).addTo(map);
var osmUrl = 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
    osmAttrib = '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors',
    osm = new L.tileLayer(osmUrl, {'attribution': osmAttrib, minZoom: 3, maxZoom: 19}).addTo(map);
    map.setView(new L.LatLng(25.0338, 121.5646), 20 );
    

var myIcon = L.icon({
    iconUrl: 'data:image/png;base64,'
})
// object.marker = L.marker([25.0338, 121.5646], {icon: myIcon}).addTo(map);

function onlineDevice(){
    ajax('GET', '/node/gps/'+object.licensePlate, {}, function(a){
        if(!a.error){
            const _ts = Math.floor(Date.now() / 1000);
            
            if(Math.abs(_ts - a.timestamp) > 3){
                $('#online').css('display', 'none');
                $('#offline').css('display', 'block');
                $('#streaming').attr('src','');
            }else{
                $('#online').css('display', 'block');
                $('#offline').css('display', 'none');
                $('#streaming').attr('src','http://123.local:8080/?action=stream');
            }
                
            console.log(a);
            map.setView(new L.LatLng(a.lat, a.lng),20);

            if (theMarker != undefined) { map.removeLayer(theMarker);};
            theMarker = L.marker([a.lat,a.lng], {icon: myIcon}).addTo(map);  
            // marker.bindPopup("<b>Hello world!</b><br>I am a popup.").openPopup();

            $('#gps_Lat').text(a.lat);
            $('#gps_Lng').text(a.lng);
            $('#gps_UpdateTime').text(moment.unix(a.timestamp).format('YYYY-MM-DD HH:mm:ss'));
            object.thread = setTimeout(() => {  onlineDevice()  }, 5000);
        }else
            console.error(a.error)
    });
}


$(document).ready(function () {

    const _uid = getCookie("username");
    if(!_uid) {
        document.cookie = "page="+ window.location.href + ";";
        location.href = "./";
    }

    ajax('GET', '/node/device/user/'+_uid, {}, function(a){
        if(!a.error){
            var cur = '', car=[], list = [];
            for(const i in a.devices){
                const device = a.devices[i];
                if(device.licensePlate != cur){
                    cur = device.licensePlate;
                    list[cur] = []
                    car.push(device.licensePlate)
                }
                list[cur].push(device.uuid);
            }

            for(const i in car){
                if(i >0) $('#carList').append('<button class="btn btn-secondary cars" style="margin:0.5rem;" data-device="'+ list[car[i]] +'">'+car[i]+'</button>')
                else $('#carList').append('<button class="btn btn-primary cars" style="margin:0.5rem;"  data-device="'+ list[car[i]] +'">'+car[i]+'</button>')
            }
            object.licensePlate = car[0];
            object.device = list[car[0]];
            onlineDevice();
        }else
            console.error(a.error)
    });

    
    
    $(document).on('click', '.cars', function(e){
        $('.cars').removeClass('btn-primary');
        $('.cars').addClass('btn-secondary');
        $(this).removeClass('btn-secondary');
        $(this).addClass('btn-primary');
        object.device = $(this).data('device').split(',');
        object.licensePlate = $(this).text();

        clearTimeout(object.thread)
        onlineDevice()
    });
    
});

